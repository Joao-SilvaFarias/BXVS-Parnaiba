...
export default function BiometriaFacial({ cliente, setCliente, setBiometria }) {
    ...
    const enviarBiometria = async (embedding) => {
        ...
        try {
            ...
            if (response.ok) {
                try {
                    const login = await axios.get("https://joaofarias16.pythonanywhere.com/api/cliente", {
                        params: {
                            email: cliente.email
                        }
                    });
                    if (login.data) {
                        setCliente(login.data);
                        localStorage.setItem("cliente", JSON.stringify(login.data));
